 ...
 * - authenticated + 유효 UUID 일 때만 요청, 마운트당 1회 + refetch 뿐.
 *   재조회 트리거는 정규화된 id(fetcher identity) 변경 + refetch 로 한정한다(401·404 자동 재요청 없음).
...
export function useReportBody(
  ...
  const enabled = status === "authenticated" && validId !== null;

  const fetcher = useCallback(
    (signal: AbortSignal): Promise<ReportBodyResult> => fetchReportBody(validId, signal),
    [validId],
  );
